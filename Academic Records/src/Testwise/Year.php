 ...
 *
 * These methods never decide the exported value on their own. They produce a
...
class Year
{
    ...
    {
        return [
            self::REGION_ENGLAND => __('England'),
            self::REGION_SCOTLAND => __('Scotland'),
            self::REGION_NORTHERN_IRELAND => __('Northern Ireland'),
            self::REGION_ROI => __('Republic of Ireland'),
            self::REGION_INTERNATIONAL => __('International'),
        ];
    }
     ...
     *
     * @return string One of the self::REGION_* values.
     */
    public static function defaultRegion(string $country): string
    {
        $country = strtolower(trim($country));

        if ($country === 'united kingdom') {
            return self::REGION_ENGLAND;
        }

        if ($country === 'ireland') {
            return self::REGION_ROI;
        }

        return self::REGION_INTERNATIONAL;
    }
     ...
     */
    public static function parseYearGroup(string $name, string $nameShort): array
    {
     ...
     * @return bool
     */
    public static function isGradeScaleName(string $name, string $nameShort): bool
    {
     ...
     *
     * @param string $region    One of the self::REGION_* values.
     * @param string $name      gibbonYearGroup.name.
     * @param string $nameShort gibbonYearGroup.nameShort.
     *
     * @return string|null Suggested value such as 'Y7', or null if none can be read.
     */
    public static function suggestYear(string $region, string $name, string $nameShort): ?string
    {
        $parsed = self::parseYearGroup($name, $nameShort);
        ...
        switch ($region) {
            ...
            case self::REGION_INTERNATIONAL:
                ...
                if (self::isGradeScaleName($name, $nameShort)) {
                    return 'Y' . ($number + 1);
                }

                return 'Y' . $number;

            case self::REGION_ENGLAND:
            case self::REGION_ROI:
            default:
                return 'Y' . $number;
        }
    }
     ...
     */
    public static function recognisedScale(string $name, string $nameShort): ?string
    {
        if (self::isGradeScaleName($name, $nameShort)) {
            return 'grade';
        }
     ...
     */
    public static function suggestYearMap(string $region, array $yearGroups): array
    {
        $suggestions = [];
        $unrecognised = [];
        $offsets = [];
        ...
        foreach ($yearGroups as $yearGroup) {
            ...
            if (self::recognisedScale($name, $nameShort) === null) {
                $suggestions[$yearGroupID] = null;
                $unrecognised[$yearGroupID] = $sequence;
                continue;
            }

            $value = self::suggestYear($region, $name, $nameShort);
     ...
     * @return bool
     */
    public static function isValidYear(string $value): bool
    {
     ...
     */
    public static function normaliseYear(string $value): string
    {
